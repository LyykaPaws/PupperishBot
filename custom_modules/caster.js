...
module.exports = {
	shoutout: function (target,context,client,channel){ // Set up to begin our module
		if (context['user-type'] === 'mod' || context.username === channel) { //Check if user is mod or channel owner
			if (target.startsWith('@')){ // If target of shoutout starts with @
				...
				console.log(`* ${context.username} used shoutout. Target: ${target}`) // Log the shoutout and who did it
			} else {
			...
			console.log(`${context.username} cannot use shoutout.`); // Log that a user with insufficient permissions tried to shout someone out.
		}
	}
};